...
static int mips_5kc_bulk_write_memory(struct target *target, target_ulong address,
...
static int mips_5kc_halt(struct target *target)
{
	...
	if (target->state == TARGET_RESET) {
		if ((jtag_get_reset_config() & RESET_SRST_PULLS_TRST) && jtag_get_srst()) {
			 ...
			 * debug entry was already prepared in mips_5kc_assert_reset()
			 */
...
static int mips_5kc_restore_smp(struct target *target, uint64_t address, int handle_breakpoints)
{
	int retval = ERROR_OK;
	struct target_list *head;
	struct target *curr;
		...
		target_ulong address, int handle_breakpoints, int debug_execution)
{
	...
	if (retval == ERROR_OK && target->smp) {
		...
		retval = mips_5kc_restore_smp(target, address, handle_breakpoints);
	}
	...
	if (breakpoint->type == BKPT_HARD) {
		...
		if (breakpoint->length == 4) {
			...
			if (verify != MIPS64_SDBBP) {
				LOG_ERROR("Unable to set 32bit breakpoint at address 0x%" PRIX
			...
			if (verify != MIPS16_SDBBP) {
				LOG_ERROR("Unable to set 16bit breakpoint at address 0x%" PRIX
						" - check that memory is read/writable", breakpoint->address);
				return ERROR_OK;
			}
		}
...
static int mips_5kc_add_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	struct mips64_common *mips64 = target_to_mips64(target);
...
static int mips_5kc_add_watchpoint(struct target *target, struct watchpoint *watchpoint)
{
	struct mips64_common *mips64 = target_to_mips64(target);
		...
		uint32_t size, uint32_t count, const uint8_t *buffer)
{
	...
	if (size == 8 && count > 64) {
		int retval = mips_5kc_bulk_write_memory(target, address, count, buffer);
		if (retval == ERROR_OK)
			return ERROR_OK;
...
{
	struct mips_5kc_common *mips_5kc = calloc(1, sizeof(struct mips_5kc_common));
		...
		uint32_t count, const uint8_t *buffer)
{
	...
	if (mips64->fast_data_area == NULL) {
		...
		if (retval != ERROR_OK) {
			...
			return mips_5kc_write_memory(target, address, 4, count, buffer);
		}
	...
	if (retval != ERROR_OK) {
		...
		retval = mips_5kc_write_memory(target, address, 4, count, buffer);
	}
